...
public class CreateRtcTokenPanel extends JPanel {
	...
	public CreateRtcTokenPanel() {
		...
		channelNameField = new JTextField(cnameFromProperties, 21);
		...
		createToken = new JButton("创建 RTC Token");
		...
		createToken.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (null == appId || appId.equals("")) {
					...
					if (null == expireTime || expireTime.equals("") || Integer.parseInt(expireTime) == 0) {
						expireTimestamp =(int) (new Date().getTime() / 1000) + 86400;
					}else {
						expireTimestamp =(int) (new Date().getTime() / 1000) + Integer.parseInt(expireTime);
					}
					 
					String token = "";
					try {
						token = BuilderTokenSample.generateToken(appId, appCertificate, channelName, uid,
								expireTimestamp);
					} catch (Exception e1) {
					...
					displayArea.append("AppId: " + appId + "\n" + "App Certificate: " + appCertificate + "\n"
							...
							+ DateTools.timestampToDate(expireTimestamp));
				}
			}
		});
